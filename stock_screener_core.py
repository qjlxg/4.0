...
def check_mode_1(df_recent, df_full):
    """
    模式一：底部反转启动型 (买入机会) - 优化：要求 MACD 红柱极小 (启动初期)
    """
    ...
    macd_signal = is_golden_cross and is_recent_cross and is_low_macd
    
    # **优化点 1: 要求 MACD 红柱极小 (启动初期)**
    is_early_stage = (last['MACD'] > 0) and (last['MACD'] < 0.05) 

    # 2. 地量蓄势 + 放量突破
    vol_history = df_recent['成交量'].iloc[:-1]
    is_low_vol_acc = (last['成交量'] > vol_history.mean() * 1.5) and (vol_history.min() < vol_history.mean() * 0.5)

    # 3. 突破 (收盘价突破 MA30 且当日涨幅大于 5%)
    is_breakout = last['收盘'] > last['MA30'] and last['收盘'] > prev['收盘'] * 1.05 

    return macd_signal and is_early_stage and is_low_vol_acc and is_breakout

def check_mode_2(df_recent, df_full):
    """
    模式二：强势股整理再加速型 (买入机会) - 优化：严格要求紧贴 10 日线
    """
    if len(df_recent) < NUM_DAYS_LOOKBACK:
        return False
...
def main():
    ...
    final_df = final_df[['code', 'name', 'mode']]
